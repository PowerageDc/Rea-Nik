...
local function DemoWindowWidgetsComboBoxes()
  ...
  local combo_items_str = 'aaaa\0bbbb\0cccc\0dddd\0eeee\0'
  rv,widgets.combos.current_item2 = ImGui.Combo(ctx, 'combo 3 (one-liner)', widgets.combos.current_item2, combo_items_str)

  -- Simplified one-liner Combo() using an array of const char*
  -- If the selection isn't within 0..count, Combo won't display a preview
  rv,widgets.combos.current_item3 = ImGui.Combo(ctx, 'combo 4 (out of range)', widgets.combos.current_item3, combo_items_str)
    ...
    for n,name in ipairs(widgets.dragdrop.names) do
      ...
      if ImGui.BeginDragDropTarget(ctx) then
        ...
        if rv then
          ...
          if payload_n ~= nil and payload_n >= 1 and payload_n <= #widgets.dragdrop.names then
...
function demo.DemoWindowTables()
  ...
  if ImGui.TreeNode(ctx, 'Sorting') then
    ...
    if ImGui.BeginTable(ctx, 'table_sorting', 4, tables.sorting.flags, 0.0, TEXT_BASE_HEIGHT * 15, 0.0) then
      -- Declare columns
      -- Define column IDs used as user_id in TableSetupColumn to avoid undefined globals
      local MyItemColumnID_ID, MyItemColumnID_Name, MyItemColumnID_Action, MyItemColumnID_Quantity = 0, 1, 2, 3
  ...
  if ImGui.TreeNode(ctx, 'Advanced') then
    ...
    if ImGui.BeginTable(ctx, 'table_advanced', 6, tables.advanced.flags, w, h, inner_width_to_use) then
      -- Define column IDs used as user_id in TableSetupColumn to avoid undefined globals
      local MyItemColumnID_ID, MyItemColumnID_Name, MyItemColumnID_Action, MyItemColumnID_Quantity, MyItemColumnID_Description, MyItemColumnID_Hidden = 0, 1, 2, 3, 4, 5
      -- Declare columns
      -- We use the "user_id" parameter of TableSetupColumn() to specify a user id that will be stored in the sort specifications.
      -- This is so our sort function can identify a column given our own identifier. We could also identify them based on their index!
